...
Use `.ls-connector` on an inline SVG or simple wrapper. Style paths with `.ls-connector__path`; optional `__label` and `__marker`. Attributes: `data-ls-variant`, `data-ls-tone`, `data-ls-orientation`. Variables: `--ls-connector-stroke`, `--ls-connector-stroke-width`, `--ls-connector-dash`. Decorative connectors should use `aria-hidden="true"`; meaningful connectors need text labels/descriptions.
